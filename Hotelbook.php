<?php
include('navbar.php');
include('db_config.php');
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Book Now</title>
    <link rel="shortcut icon" href="favicon.ico">


    <link href='https://fonts.googleapis.com/css?family=Open+Sans:400,700,300' rel='stylesheet' type='text/css'>
    <link rel="stylesheet" href="css/animate.css">
    <link rel="stylesheet" href="css/icomoon.css">
    <link rel="stylesheet" href="css/bootstrap.css">
    <link rel="stylesheet" href="css/superfish.css">
    <link rel="stylesheet" href="css/magnific-popup.css">
    <link rel="stylesheet" href="css/cs-select.css">
    <link rel="stylesheet" href="css/cs-skin-border.css">
    <link rel="stylesheet" href="css/style.css">
    <link rel="stylesheet" href="datetimepicker/datetimepicker-master/build/jquery.datetimepicker.min.css">
    <link rel="stylesheet" href="css\as-alert-message.min.css">
    <script src="js\jquery.min.js"></script>
   
    <script src="js/jquery.easing.1.3.js"></script>
    <script src="datetimepicker/datetimepicker-master/build/jquery.datetimepicker.full.min.js"></script>
    <script src="js\as-alert-message.min.js"></script>


    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>
    <script src="js/modernizr-2.6.2.min.js"></script>
    <style>
        .forming {
            border-radius: 5px;
            background-color: orange;
            padding: 20px;
            margin-left: 200px;
            margin-right: 200px;
            color: black;
        }

        h3,
        p,
        h4 {
            color: black;
        }

        .reserve {

            background-color: orangered;
            border: none;
            color: white;
            padding: 15px 20px;
            text-align: center;
            font-size: 16px;
            float: right;
        }
    </style>
    <script>
    var plan="";
    var text=$("#coupon").val();
        var amount;
        var check="";
        $(document).ready(function() {
            
                
            var hid = <?php echo $_GET["hid"]; ?>;
            console.log(hid);
            $.post("admin/booking_management.php", {
                    "opt": "cal",
                    "hid":<?php echo $_GET["hid"]; ?>
                },
                function(data, textStatus, jqXHR) {
                    console.log(data);
                    data = JSON.parse(data);
                    plan=data;
                    console.log(data);

                    var i=0;
                    $.each(data, function(key, value) {
                        $('#type')
                            .append($("<option></option>")
                                .attr("value", i)
                                .text(value.category));
                                i++;
                    });
                    $("select.type").change(function(){
                  
                    var ind=$(this).children("option:selected").val();
                    var fare=plan[ind].price;
                    var start =$('#atime').datetimepicker('getValue')
                    var end = $('#dtime').datetimepicker('getValue');

                    var diff = Math.round((end-start)/(1000*60*60*24));
                    var room=$('#room').val();
                    console.log(fare);
                    console.log(diff);
                    console.log(room);
                               
                     amount=fare*diff*room;
                    var str = amount;
                    $("#amt").html(str);
                    $('#amnt').val(str);
                  
                     });
                     $('#atime').datetimepicker({
                        inline:false,
                        minDate:'0',//yesterday is minimum date(for today use 0 or -1970/01/01)
                        onSelectDate:function()
                        {
                            console.log($('#atime').datetimepicker('getValue'));
                        }
                      
 
                            });
                        });
                     $('#dtime').datetimepicker({
                        inline:false,
                        minDate:'-1970/01/01',//yesterday is minimum date(for today use 0 or -1970/01/01)
                        onSelectDate:function()
                        {
                            console.log($('#dtime').datetimepicker('getValue'));
                            
                            }
                        });
                        $(".code").click(function(){
                        $.get("admin/booking_management.php", {
                            "opt": "check",
                            "uid":<?php echo $_COOKIE["user_id"];?>,
                            "code": $("#coupon").val()
                        },
                                function(data, textStatus, jqXHR) {
                                    console.log(data);
                                  data = JSON.parse(data);
                                  console.log(data);
                                  if (data.result == "true") 
                                    {
                                       
                                            
                                            asAlertMsg({
                                                    type: "success",
                                                    title: "Coupon Code",
                                                    message: "Your Coupon code is applied!!!",
                                                    button: {
                                                        title: "Close Button",
                                                        bg: "success"
                                            }
                                            })
                                            amount=amount-500;
                                            console.log(amount);
                                            $("#amt").html(amount);
                                            $('#amnt').val(amount);
                                    }
                                    else
                                    {
                                        asAlertMsg({
                                                type: "error",
                                                title: "Coupon Code",
                                                message: "This code is not valid!!!!",
                                                button: {
                                                    title: "Close Button",
                                                    bg: "error"
                                                }
                                                });
                                    }
                                },

                            );
                    })
                        
                    $(".add").click(function()

                        {

                            $.post("admin/booking_management.php", $("#reserve").serialize() + "&opt=add",
                                function(data) {
                                    console.log(data);
                                    data = JSON.parse(data);
                                    console.log(data);
                                    if (data.result == "true") {
                                        $.notify("Booking added", "success", function() {
                                            location.reload();

                                        });
                                    } else {
                                        $.notify("Error", "error");
                                    }
                                },

                            );

                        });
                });
       
    </script>
</head>

<body>
    <div class="forming">

        <h3>Hotel Reservation Form</h3>
        <p> Please complete the form below to reserve in hotel
            <?php


            echo $_GET["hname"];

            ?></p>
        <hr>

        <form id="reserve">
            <input type="hidden" name="h_id" id="h_id" value="<?php echo $_GET["hid"]; ?>" />
            <input type="hidden" name="hname" id="hname" value="<?php echo $_GET["hname"]; ?>" />
            <input type="hidden" name="u_id" id="u_id" value="<?php echo $_COOKIE['user_id']; ?>" />
            <label> Full Name</label><br>
            <input type="text" id="fname" name="fname" /><br>
            <label> Phone number</label><br>
            <input type="text" id="phno" name="phno" /><br>
            <label> Email </label> <br>
            <input type="email" id="mail" name="mail" /><br>
            <hr>
            <label> Arriving date </label><br>
            <input type="text" id="atime" name="atime"/>

            <br>
            <label> Departure date </label><br>
            <input type="text" id="dtime" name="dtime"/>
             <br>
            <hr>
            <h3>Accomodation</h3>
            <label>Number of rooms you want to book</label><br>
            <input type="text" id="room" name="room" /><br>
            <label>Number of Adults</label><br>
            <input type="text" id="adult" name="adult" /><br>
            <label>Number of Children</label><br>
            <input type="text" id="child" name="child" /><br>
            <label> Which type of room you are booking</label><br>
            <select name="type" id="type" class="type"><br>
            <option value="none" selected disabled hidden>
                Select an Option
            </option>
            </select>
            <h4>Any Special Request</h4>
            <textarea id="req" name="req"></textarea>
            <hr>
            <h3>Payment Method</h3>
            <label>Cash</label>
            <input type="radio" name="pay" id="cash" value="cash"><br>
            <label>Net Banking</label>
            <input type="radio" name="pay" id="net" value="net"><br>
            <label> Apply Coupon Code</label><br>
            <input type="text" id="coupon" name="coupon" /><br>
            <button type="button" class="code">Check</button><br>
            <label>You have to pay </label>
            <label id="amt" name="amt"></label>
            <input type="hidden" name="amnt" id="amnt"/>
            <button type="button" class="reserve add">Reserve</button>
            <br>
            <br>
        </form>
    </div>
</body>

</html>